    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6">
                <form action="{{ route('export-templates.store') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" required
                            class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-blue-500 focus:border-blue-500"
                            placeholder="このテンプレートの用途を記入">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            @foreach (['csv' => 'CSV', 'excel' => 'Excel', 'json' => 'JSON', 'xml' => 'XML'] as $value => $label)
                                <label
                                    ...
                                    {{ old('format') === $value ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30' : 'border-gray-300 dark:border-gray-600' }}">
                                    <input type="radio" name="format" value="{{ $value }}"
                                        {{ old('format') === $value ? 'checked' : '' }} required class="sr-only peer">
                                    <span
                            ...
                            @foreach ($availableFields as $value => $label)
                                <label
                                    ...
                                    <input type="checkbox" name="fields[]" value="{{ $value }}"
                                        {{ is_array(old('fields')) && in_array($value, old('fields')) ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <select name="order[]" id="order" multiple size="5"
                            ...
                            @foreach ($availableFields as $value => $label)
                                <option value="{{ $value }}"
                                    {{ is_array(old('order')) && in_array($value, old('order')) ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                            ...
                            <div>
                                ...
                                <select name="filters[status]" id="filter_status"
                                    ...
                                    <option value="completed"
                                        {{ old('filters.status') === 'completed' ? 'selected' : '' }}>完了のみ</option>
                                    <option value="active" {{ old('filters.status') === 'active' ? 'selected' : '' }}>
                                        未完了のみ</option>
                                </select>
                            </div>
                        </div>
